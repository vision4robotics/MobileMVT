...
class Mapper(object):
    ...
    def apply_delta_pose(self, delta_R=np.eye(3), delta_T=np.zeros((3, 1))):
        Ko_new = self.Ko.copy()
        Ko_new[:3, :3] = delta_R @ self.Ko[:3, :3]
        Ko_new[:3, 3:] = self.Ko[:3, 3:] + delta_T
...
def estimate_pose_from_box_centers(prev_boxes, curr_boxes):
    if len(prev_boxes) < 3 or len(curr_boxes) != len(prev_boxes):
        return np.eye(3), np.zeros((3, 1))

    prev_pts = np.array([[x + w / 2, y + h] for x, y, w, h in prev_boxes], dtype=np.float32)
    curr_pts = np.array([[x + w / 2, y + h] for x, y, w, h in curr_boxes], dtype=np.float32)

    H, mask = cv2.estimateAffinePartial2D(prev_pts, curr_pts)
    if H is None:
        return np.eye(3), np.zeros((3, 1))
